...
export default function StepWheels() {
  ...
  // Reset confirmation when wheels change
  useEffect(() => {
    setIsConfirmed(false);
  }, [selectedWheels]);

  // Filtrar productos por categoría rueda
  const availableWheels = PRODUCTS.filter(p => p.component_type === 'rueda');

  // Seleccionar la primera por defecto si no hay ninguna seleccionada
  useEffect(() => {
    if (!selectedWheels && availableWheels.length > 0) {
      setWheels(availableWheels[0]);
    }
  }, [selectedWheels, availableWheels, setWheels]);

  const currentWheels = selectedWheels || availableWheels[0];
  ...
  if (!currentWheels) return null;

  return (    <div className="relative max-w-6xl mx-auto w-full min-h-[700px] flex flex-col items-center py-12 md:py-16 overflow-hidden">
      ...
      <div className="relative z-10 w-full text-center mb-12 md:mb-16">
        ...
        >
          El punto de contacto con el mundo. El grip y el deslizamiento perfecto para tu disciplina.
        </motion.p>
      </div>
      ...
      <div className="relative z-30 w-full max-w-6xl px-4 mb-12 md:mb-20">
        <div className="flex items-center justify-between mb-8 px-6">
          <span className="text-[#1C1612] text-[10px] font-black uppercase tracking-[0.4em] opacity-40">Uretano de Alta Gama</span>
        ...
        >
          {availableWheels.map((wheel, i) => {
            ...
            return (
              ...
              >
                <div className="h-24 md:h-36 mb-4 flex items-center justify-center p-4">
                  <img 
                ...
                <div className="text-center">
                  ...
                  )}>{wheel.brand_name}</span>
                  <span className="text-xs md:text-sm font-black text-[#1C1612] block truncate leading-tight">{wheel.name}</span>
                </div>
                
                {isSelected && (
                  <motion.div 
                    layoutId="check-wheels-top"
      ...
      <div className="relative z-10 grid md:grid-cols-2 gap-10 md:gap-16 items-center w-full mb-12 md:mb-20 px-8">
        ...
        <div className="flex flex-col items-center justify-center order-2 md:order-1">
          <AnimatePresence mode="wait">
            ...
            >
              <RadarChart data={currentWheels.specs || []} size={280} className="md:scale-110" />
            </motion.div>
          </AnimatePresence>
        </div>

        {/* Wheels Photo Section */}
        <div className="flex items-center justify-center order-1 md:order-2">
          <AnimatePresence mode="wait">
            ...
            >
              <div className="absolute inset-0 bg-[#D97230]/5 blur-[150px] rounded-full scale-150 opacity-40 group-hover:opacity-60 transition-opacity duration-1000" />
              
              <div className="relative z-10 bg-white p-6 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.06)] border border-[#F5E1E1]/30 overflow-hidden">
                <img 
                  src={currentWheels.images?.[0]?.url ?? '/images/placeholder.png'} 
                  alt={currentWheels.name} 
                  className="relative z-10 max-h-[220px] md:max-h-[400px] w-full object-contain drop-shadow-[0_30px_60px_rgba(0,0,0,0.08)] hover:scale-110 transition-transform duration-1000 ease-out"
                />
              </div>
            </motion.div>
          </AnimatePresence>
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              {isConfirmed ? 'Ruedas Seleccionadas' : 'Seleccionar estas ruedas'}
              <div className={cn(
